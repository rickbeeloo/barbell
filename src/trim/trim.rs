...
impl LabelConfig {
    ...
    fn create_label(&self, annotations: &[Match]) -> String {
...
struct CompleteSlice {
    ...
    annotations: Vec<Match>,  // Store all relevant annotations
}

fn preprocess_cuts(annotations: &[Match], seq_len: usize) -> Vec<CompleteSlice> {
    let mut slices: Vec<CompleteSlice> = Vec::new();
    
    // Group cuts by their IDs
    let mut cut_groups: HashMap<usize, Vec<(usize, usize, &Cut, &Match)>> = HashMap::new();
...
pub fn process_read_and_anno(seq: &[u8], qual: &[u8], annotations: &[Match], label_config: &LabelConfig) -> Vec<(Vec<u8>, Vec<u8>, String, String)> {
    ...
    for slice in &slices {
            ...
            let label_matches: Vec<Match> = slice.annotations.clone();
...
pub fn trim_matches(filtered_match_file: &str, read_fastq_file: &str, output_folder: &str, add_labels: bool, add_orientation: bool, add_flank: bool, sort_labels: bool) {
    ...
    println!("  • Sort labels:   {}", if sort_labels { "yes".green() } else { "no".red() });
    ...
    let mut current_annotations: Vec<Match> = Vec::new();

    for result in matches_reader.deserialize() {
        let anno: Match = result.expect("Failed to parse annotation line");
